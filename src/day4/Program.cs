...
static int FindString(List<string> lines, string letters)
{
    ...
    for (var i = 0; i < lines.Count; i++)
    {
        for (var j = 0; j < lines[i].Length; j++)
        {
            ...
            {
                // start looking here out each direction.
                // forwards
                // if (lines[i][j..].StartsWith(letters))
                // {
                //     result++;
                // }
                //
                // if (j - letters.Length >= 0 && lines[i]
                //         .Substring(j - letters.Length)
                //         .StartsWith(letters.Reverse().ToString()))
                // {
                //     result++;
                // }
